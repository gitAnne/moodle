...
require('../../config.php');
require_once($CFG->dirroot.'/enrol/url/locallib.php');
...
$instance = $DB->get_record('enrol', array('id'=>$enrolid, 'enrol'=>'url'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id'=>$instance->courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

require_login($course);
$canenrol = has_capability('enrol/url:enrol', $context);
$canunenrol = has_capability('enrol/url:unenrol', $context);
...
if (!$canenrol and !$canunenrol) {
    ...
    require_capability('enrol/url:enrol', $context);
    require_capability('enrol/url:unenrol', $context);
}
...
if (!$enrol_url = enrol_get_plugin('url')) {
    throw new coding_exception('Can not instantiate enrol_url');
}

$instancename = $enrol_url->get_instance_name($instance);

$PAGE->set_url('/enrol/url/manage.php', array('enrolid'=>$instance->id));
$PAGE->set_pagelayout('admin');
$PAGE->set_title($enrol_url->get_instance_name($instance));
...
$potentialuserselector = new enrol_url_potential_participant('addselect', $options);
$currentuserselector = new enrol_url_current_participant('removeselect', $options);
...
if ($canenrol && optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        foreach($userstoassign as $adduser) {
            ...
            $enrol_url->enrol_user($instance, $adduser->id, $roleid, $timestart, $timeend);
        }
...
if ($canunenrol && optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstounassign)) {
        foreach($userstounassign as $removeuser) {
            $enrol_url->unenrol_user($instance, $removeuser->id);
        }
    ...
    <tr>
      ...
      <td id="buttonscell">
          <div id="addcontrols">
              ...
              <p><label for="menuroleid"><?php print_string('assignrole', 'enrol_url') ?></label><br />
