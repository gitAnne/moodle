...
/**
 * URL  enrolment plugin.
 *
 * This plugin Allows people to enrol automaitccaly while calling a url with the course and section as paramters.
 * 
 * @package    enrol_url
...
/**
 * URL enrolment plugin implementation.
...
class enrol_url_plugin extends enrol_plugin {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol_user($instance, $ue) && has_capability('enrol/url:unenrol', $context)) {
    ...
    /**
     * Forces synchronisation of user enrolments with external url,
    ...
    public function sync_user_enrolments($user) {
        ...
        if ($rs = $extdb->Execute($sql)) {
            if (!$rs->EOF) {
                while ($fields = $rs->FetchRow()) {
                    ...
                    if ($instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'url'), '*', IGNORE_MULTIPLE)) {
        ...
        foreach ($roleassigns as $courseid => $roles) {
            ...
            $current = $DB->get_records('role_assignments', array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_url', 'itemid'=>$instance->id), '', 'id, roleid');
            ...
            foreach ($current as $r) {
                if (isset($roles[$r->roleid])) {
                    ...
                    role_unassign($r->roleid, $user->id, $context->id, 'enrol_url', $instance->id);
                }
            }
            foreach ($roles as $rid) {
                if (!isset($existing[$rid])) {
                    role_assign($rid, $user->id, $context->id, 'enrol_url', $instance->id);
                }
            }
        }
        ...
        $sql = "SELECT e.*, c.visible AS cvisible, ue.status AS ustatus
             ...
             LEFT JOIN {user_enrolments} ue ON ue.enrolid = e.id
                 WHERE ra.userid = :userid AND e.enrol = 'url'";
        ...
        foreach ($rs as $instance) {
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                    ...
                    role_unassign_all(array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_url', 'itemid'=>$instance->id));
                }
            }
        }
        $rs->close();
    ...
    /**
     * Forces synchronisation of all enrolments with url
     *
    ...
    public function sync_enrolments(progress_trace $trace, $onecourse = null) {
        ...
        if (!$extdb = $this->db_init()) {
            $trace->output('Error while communicating with external enrolment url');
            $trace->finished();
            return 1;
        }
        ...
        if ($onecourse) {
            $sql = "SELECT c.id, c.visible, c.$localcoursefield AS mapping, c.shortname, e.id AS enrolid
                      FROM {course} c
                 LEFT JOIN {enrol} e ON (e.courseid = c.id AND e.enrol = 'url')
                     WHERE c.id = :id";
            ...
            $sql = "SELECT c.id, c.visible, c.$localcoursefield AS mapping, e.id AS enrolid, c.shortname
                      FROM {course} c
                      JOIN {enrol} e ON (e.courseid = c.id AND e.enrol = 'url')";
        ...
        foreach ($existing as $course) {
            ...
            $sql = "SELECT u.$localuserfield AS mapping, u.id AS userid, ue.status, ra.roleid
                      FROM {user} u
                      JOIN {role_assignments} ra ON (ra.userid = u.id AND ra.component = 'enrol_url' AND ra.itemid = :enrolid)
            ...
            if ($rs = $extdb->Execute($sql)) {
                ...
                $trace->output("error: skipping course '$course->mapping' - could not match with url", 1);
                continue;
            }
            unset($usermapping);
            ...
            foreach ($requestedroles as $userid => $userroles) {
                ...
                foreach ($userroles as $roleid) {
                    if (empty($currentroles[$userid][$roleid])) {
                        role_assign($roleid, $userid, $context->id, 'enrol_url', $instance->id);
                        $currentroles[$userid][$roleid] = $roleid;
                ...
                foreach ($currentroles[$userid] as $cr) {
                    if (empty($userroles[$cr])) {
                        role_unassign($cr, $userid, $context->id, 'enrol_url', $instance->id);
            ...
            foreach ($currentroles as $userid => $userroles) {
                // These are roles that exist only in Moodle, not the url
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                foreach ($currentstatus as $userid => $status) {
                    ...
                    if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                        ...
                        role_unassign_all(array('contextid'=>$context->id, 'userid'=>$userid, 'component'=>'enrol_url', 'itemid'=>$instance->id));
    ...
    public function sync_courses(progress_trace $trace) {
        ...
        if (!$extdb = $this->db_init()) {
            $trace->output('Error while communicating with url');
            $trace->finished();
            return 1;
        }
    ...
    /**
     * Tries to make connection to the url.
     *
    ...
    public function get_newinstance_link($courseid) {
      ...
      if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/url:config', $context)) {
        return NULL;
      }
    
      if ($DB->record_exists('enrol', array('courseid'=>$courseid, 'enrol'=>'url'))) {
        return NULL;
      }
    
      return new moodle_url('/enrol/url/edit.php', array('courseid'=>$courseid));
    }
